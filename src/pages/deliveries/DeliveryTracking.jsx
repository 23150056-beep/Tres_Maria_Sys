...
const statusColors = {
  scheduled: { bg: 'bg-slate-100', text: 'text-slate-500', dot: 'bg-slate-400' },
  loading: { bg: 'bg-amber-50', text: 'text-amber-600', dot: 'bg-yellow-400' },
  in_transit: { bg: 'bg-blue-50', text: 'text-blue-600', dot: 'bg-blue-400 animate-pulse' },
  delivered: { bg: 'bg-emerald-50', text: 'text-emerald-600', dot: 'bg-green-400' },
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-900/70 mt-4">Loading delivery tracking...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Real-time driver and delivery status monitoring</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <GlassCard className="p-4">
          ...
          <p className="text-2xl font-bold text-slate-900">{todayStats?.total_deliveries || 0}</p>
        </GlassCard>
        <GlassCard className="p-4">
          <p className="text-slate-500 text-sm">Scheduled</p>
          <p className="text-2xl font-bold text-slate-500">{todayStats?.scheduled || 0}</p>
        </GlassCard>
        <GlassCard className="p-4">
          <p className="text-slate-500 text-sm">In Transit</p>
          <p className="text-2xl font-bold text-blue-600">{todayStats?.in_transit || 0}</p>
        </GlassCard>
        <GlassCard className="p-4">
          <p className="text-slate-500 text-sm">Completed</p>
          <p className="text-2xl font-bold text-emerald-600">{todayStats?.completed || 0}</p>
        </GlassCard>
        <GlassCard className="p-4">
          <p className="text-slate-500 text-sm">Stop Progress</p>
          ...
          <div className="space-y-3 max-h-[600px] overflow-y-auto">
            {activeDeliveries?.length > 0 ? (
              activeDeliveries.map((delivery) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-medium text-slate-900">{delivery.delivery_number}</span>
              ...
              <div className="text-center py-8">
                <TruckIcon className="h-12 w-12 text-slate-900/20 mx-auto mb-2" />
        ...
        <GlassCard className="p-6 lg:col-span-2">
          {selectedDelivery ? (
            <div>
              ...
              <div className="bg-slate-50 rounded-xl p-4 mb-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-slate-900">{selectedDelivery.driver_name || 'Unassigned'}</p>
                    <p className="text-sm text-slate-500">{selectedDelivery.plate_number}</p>
                  </div>
                  {selectedDelivery.driver_phone && (
                    <a
                      href={`tel:${selectedDelivery.driver_phone}`}
                      className="p-2 bg-emerald-50 rounded-lg hover:bg-green-500/30 transition-colors"
                    >
              ...
              {/* Map Placeholder */}
              <div className="bg-slate-100 border border-slate-200 rounded-xl h-48 mb-4 flex items-center justify-center border border-slate-200">
                <div className="text-center">
                  <MapPinIcon className="h-12 w-12 text-slate-900/20 mx-auto mb-2" />
                  <p className="text-slate-400">Map integration available</p>
                  <p className="text-xs text-slate-900/30">GPS: {selectedDelivery.current_latitude?.toFixed(4)}, {selectedDelivery.current_longitude?.toFixed(4)}</p>
                </div>
              </div>

              {/* Delivery Stops */}
              <h4 className="font-medium text-slate-900 mb-3">Delivery Stops</h4>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-slate-900 truncate">{stop.client_name}</p>
                        <p className="text-xs text-slate-400 truncate">{stop.delivery_address}</p>
                      </div>
                      {stop.order_number && (
                        <span className="text-xs text-primary-600">{stop.order_number}</span>
                      )}
                    </div>
                  </div>
                ))}
            ...
            <div className="h-full flex items-center justify-center">
              <div className="text-center">
                <TruckIcon className="h-16 w-16 text-slate-900/20 mx-auto mb-3" />
      ...
      {driverLocations?.length > 0 && (
        <GlassCard className="p-6">
          <h3 className="text-lg font-semibold text-slate-900 mb-4">Active Drivers</h3>
            ...
            {driverLocations.map((driver) => (
              ...
              >
                ...
                {driver.delivery_number && (
                  <p className="text-sm text-primary-600">{driver.delivery_number}</p>
                )}

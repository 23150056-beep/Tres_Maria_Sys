    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">{delivery.delivery_number || `DEL-${String(delivery.id).padStart(6, '0')}`}</h1>
            <p className="text-white/60">Order: {delivery.order?.order_number}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="glass-card p-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="font-medium text-white flex items-center"><PhoneIcon className="h-4 w-4 mr-2" />{delivery.order?.client?.phone || '-'}</p>
              </div>
              <div className="col-span-2">
                <p className="text-sm text-white/60">Delivery Address</p>
                <p className="font-medium text-white flex items-start"><MapPinIcon className="h-4 w-4 mr-2 mt-1 flex-shrink-0" />{delivery.delivery_address || delivery.order?.client?.address || '-'}</p>
              </div>
            </div>
          </div>
          ...
          {/* Schedule */}
          <div className="glass-card p-6">
            <h2 className="text-lg font-semibold text-white mb-4">Schedule & Assignment</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="p-4 bg-white/5 rounded-lg">
                <ClockIcon className="h-5 w-5 text-white/40 mb-2" />
                <p className="text-sm text-white/60">Scheduled Date</p>
                <p className="font-medium text-white">{formatDate(delivery.scheduled_date)}</p>
              </div>
              <div className="p-4 bg-white/5 rounded-lg">
                <ClockIcon className="h-5 w-5 text-white/40 mb-2" />
                <p className="text-sm text-white/60">Scheduled Time</p>
                <p className="font-medium text-white">{formatTime(delivery.scheduled_date)}</p>
              </div>
              <div className="p-4 bg-white/5 rounded-lg">
                <TruckIcon className="h-5 w-5 text-white/40 mb-2" />
                <p className="text-sm text-white/60">Vehicle</p>
                <p className="font-medium text-white">{delivery.vehicle?.plate_number || '-'}</p>
              </div>
              <div className="p-4 bg-white/5 rounded-lg">
                <TruckIcon className="h-5 w-5 text-white/40 mb-2" />
                <p className="text-sm text-white/60">Driver</p>
                <p className="font-medium text-white">{delivery.driver?.name || delivery.driver_name || '-'}</p>
              </div>
            </div>
          </div>
            ...
            <table className="glass-table">
              <thead>
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-white/60 uppercase">Qty</th>
                  <th className="px-6 py-3 text-right text-xs font-medium text-white/60 uppercase">Price</th>
              ...
              <tbody className="divide-y divide-white/10">
                {delivery.order?.items?.map((item, idx) => (
                  <tr key={idx}>
                    <td className="px-6 py-4 text-white">{item.product?.name || `Product #${item.product_id}`}</td>
                    <td className="px-6 py-4 text-white">{item.quantity} {item.product?.unit}</td>
                    <td className="px-6 py-4 text-right text-white">{formatCurrency(item.unit_price)}</td>
                    <td className="px-6 py-4 text-right font-medium text-white">{formatCurrency(item.total_price)}</td>
                  </tr>
                )) || <tr><td colSpan="4" className="px-6 py-4 text-center text-white/60">No items</td></tr>}
              </tbody>
              <tfoot className="border-t border-white/10">
                <tr>
                  <td colSpan="3" className="px-6 py-4 text-right font-medium text-white">Order Total:</td>
                  <td className="px-6 py-4 text-right font-bold text-lg text-white">{formatCurrency(delivery.order?.total_amount)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={idx} className="flex items-start gap-4">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step.done ? 'bg-green-500/20' : 'bg-white/10'}`}>
                    {step.done ? <CheckCircleIcon className="h-5 w-5 text-green-400" /> : <div className="w-2 h-2 rounded-full bg-white/40"></div>}
                  </div>
                  <div>
                    <p className={`font-medium ${step.done ? 'text-white' : 'text-white/40'}`}>{step.label}</p>
                    <p className="text-sm text-white/60">{step.time ? `${formatDate(step.time)} at ${formatTime(step.time)}` : 'Pending'}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-6">
          ...
          {/* Proof of Delivery */}
          <div className="glass-card p-6">
            <h2 className="text-lg font-semibold text-white mb-4">Proof of Delivery</h2>
            {delivery.status === 'delivered' ? (
              <div className="space-y-4">
                <div className="p-4 bg-white/5 rounded-lg text-center">
                  <CameraIcon className="h-8 w-8 text-white/40 mx-auto mb-2" />
                  <p className="text-sm text-white/60">Delivery photo captured</p>
                </div>
                <div>
                  <p className="text-sm text-white/60">Received by</p>
                  <p className="font-medium text-white">{delivery.received_by || 'N/A'}</p>
                </div>
                {delivery.signature_url && (
                  <div>
                    <p className="text-sm text-white/60 mb-2">Signature</p>
              ...
              <p className="text-sm text-white/60 text-center py-4">Proof of delivery will be available once delivered</p>
            )}
          </div>
          ...
          <div className="glass-card p-6">
            ...
            <p className="text-white/60">{delivery.notes || 'No delivery notes'}</p>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4">
            ...
            <div className="relative glass-card max-w-md w-full p-6">
              <h2 className="text-xl font-bold text-white mb-4">Mark Delivery as Failed</h2>
              <div className="mb-4">
                <label className="block text-sm font-medium text-white/60 mb-2">Reason for Failure *</label>

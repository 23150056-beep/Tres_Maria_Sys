    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500">Revenue, expenses, and profitability analysis</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-4">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <select value={filters.groupBy} onChange={(e) => setFilters({ ...filters, groupBy: e.target.value })} className="px-3 py-1.5 border border-gray-300 rounded-lg text-sm">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <ResponsiveContainer width="100%" height={350}>
          <ComposedChart data={data?.trend || []}>
            ...
            <Tooltip formatter={(v, name) => name === 'Profit Margin' ? `${v}%` : formatCurrency(v)} labelFormatter={(d) => new Date(d).toLocaleDateString('en-PH')} />
            <Legend />
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center p-4 bg-green-50 rounded-lg">
              ...
              <span className="text-xl font-bold text-green-600">{formatCurrency(data?.receivables?.total)}</span>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div className="p-3 bg-gray-50 rounded-lg text-center">
                <p className="text-xs text-gray-500">Current</p>
                <p className="font-semibold text-green-600">{formatCurrency(data?.receivables?.current)}</p>
              </div>
              <div className="p-3 bg-yellow-50 rounded-lg text-center">
                <p className="text-xs text-gray-500">30+ Days</p>
                <p className="font-semibold text-yellow-600">{formatCurrency(data?.receivables?.days30)}</p>
              </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="p-3 bg-gray-50 rounded-lg text-center">
                <p className="text-xs text-gray-500">Current</p>
                <p className="font-semibold">{formatCurrency(data?.payables?.current)}</p>
              </div>
              <div className="p-3 bg-yellow-50 rounded-lg text-center">
                <p className="text-xs text-gray-500">30+ Days</p>
                <p className="font-semibold text-yellow-600">{formatCurrency(data?.payables?.days30)}</p>
              </div>
              <div className="p-3 bg-red-50 rounded-lg text-center">
                <p className="text-xs text-gray-500">60+ Days</p>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <AreaChart data={data?.cashFlow || []}>
            ...
            <Tooltip formatter={(v) => formatCurrency(v)} labelFormatter={(d) => new Date(d).toLocaleDateString('en-PH')} />
            <Legend />
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="p-6 border-b">
          <h3 className="text-lg font-semibold">Recent Large Transactions</h3>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Client/Supplier</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {(data?.recentTransactions || []).slice(0, 10).map((tx, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  <td className="px-6 py-4 text-sm">{new Date(tx.date).toLocaleDateString('en-PH')}</td>
                  ...
                  <td className="px-6 py-4 text-gray-500">{tx.entity || '-'}</td>
              ...
              {(!data?.recentTransactions || data.recentTransactions.length === 0) && (
                <tr><td colSpan="5" className="px-6 py-12 text-center text-gray-500">No transactions in this period</td></tr>
              )}
            </tbody>
          </table>
        </div>

    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4">
          ...
          <p className="text-2xl font-bold">{formatCurrency(data?.summary?.avgOrderValue)}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm p-4">
          <p className="text-sm text-gray-500">Items Sold</p>
          <p className="text-2xl font-bold">{formatNumber(data?.summary?.totalItems)}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <h3 className="text-lg font-semibold mb-4 flex items-center"><ChartBarIcon className="h-5 w-5 mr-2 text-blue-600" />Sales Trend</h3>
          ...
          <div className="space-y-3">
            {(data?.topClients || []).slice(0, 10).map((client, idx) => (
              <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                ...
                <span className="font-semibold text-green-600">{formatCurrency(client.total)}</span>
              </div>
            ))}

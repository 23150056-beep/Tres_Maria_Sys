  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <button onClick={() => navigate('/users')} className="p-2 text-slate-500 hover:bg-slate-100 rounded-lg">
          <ArrowLeftIcon className="h-5 w-5" />
        </button>
        <h1 className="text-2xl font-bold text-slate-900">{isEdit ? 'Edit User' : 'Add New User'}</h1>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="glass-card p-6">
          <h2 className="text-lg font-semibold text-slate-900 mb-4 flex items-center">
            <UserIcon className="h-5 w-5 mr-2 text-primary-600" />Basic Information
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-slate-500 mb-1">{isEdit ? 'New Password (leave blank to keep current)' : 'Password *'}</label>
        ...
        <div className="glass-card p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {roles.map(r => (
              <label key={r.value} className={`relative flex items-start p-4 border rounded-lg cursor-pointer ${role === r.value ? 'border-primary-500 bg-primary-50' : 'border-slate-200 hover:bg-slate-50'}`}>
                ...
                {role === r.value && (
                  <div className="absolute top-2 right-2 h-5 w-5 bg-primary-600 rounded-full flex items-center justify-center">
                    <svg className="h-3 w-3 text-slate-900" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
                  </div>
                )}
              </label>
            ))}
        ...
        <div className="glass-card p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-slate-500 mb-1">Assigned Warehouse</label>
              <select {...register('warehouse_id')} className="form-select">
                <option value="">All Warehouses (Admin/Manager)</option>
                {warehouses.map(w => <option key={w.id} value={w.id}>{w.name} ({w.code})</option>)}
              </select>
              <p className="mt-1 text-xs text-slate-400">Warehouse staff should be assigned to a specific warehouse</p>
            </div>
            <div className="flex items-center">
              <label className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" {...register('is_active')} className="h-5 w-5 text-primary-600 rounded" />
                <div>
                  <p className="font-medium text-slate-900">Active Account</p>
                  <p className="text-sm text-slate-500">Inactive users cannot log in</p>
                </div>
              </label>
            </div>
          </div>
